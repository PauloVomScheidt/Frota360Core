...
- `permissions.ts` (`pode.*`) espelha a matriz de papéis da API (Admin / Supervisor / Operador / Motorista — um motorista é um usuário com essa role, não um cadastro à parte) puramente para esconder ações que dariam 403 — **o servidor é sempre a autoridade de fato**; nunca confie na matriz do cliente para lógica sensível a segurança. ⚠️ `pode.excluir` é **Admin e Supervisor**: exclusão acompanha edição em toda tela de gestão. O que segue exclusivo do Admin são as telas dele — `/usuarios`, `/convites` e `/auditoria`. ⚠️ `pode.editarTiposCombustivel`/`pode.editarPostos` escondem só as **telas** `/tipos-combustivel` e `/postos`: na API a **leitura** desses dois catálogos é aberta a todos os papéis, porque o motorista precisa deles para lançar abastecimento — por isso `AbastecimentosPage` os busca sem `enabled`, ao contrário de `['motoristas']`.
