...
- **Catálogo semeado tem duas fontes que precisam andar juntas.** `TiposManutencaoPadrao`/`TiposDespesaPadrao` cobrem empresa nova via `BackofficeService`; as empresas já existentes foram semeadas por `INSERT` no `Up` da migration correspondente. A lista fica duplicada de propósito — migration é histórica e não acompanha a constante. Ao criar um catálogo novo, lembre dos **dois** caminhos: sem o da migration, todo cliente atual abre a tela com o seletor vazio.
