...
- **Repositório de read model é exceção, não regra.** `ICustoRepository` atravessa `Abastecimento`, `Manutencao` e `Rota` porque serve uma consulta, não um agregado — e não escreve nada. Fora esse caso, mantenha um repositório por agregado.
