...
`AddInfrastructure` lança na inicialização se `Jwt:Key` faltar ou tiver menos de 32 caracteres. Em produção: `Jwt__Key`, `Resend__ApiKey`, `Backoffice__ApiKey`, `GoogleRoutes__ApiKey`, `ConnectionStrings__DefaultConnection` por variável de ambiente.
...
- **O odômetro do veículo só avança.** **Três** fluxos o movem — abertura/encerramento de rota, conclusão de manutenção e **lançamento/correção de abastecimento** — e os três comparam antes de gravar: um lançamento retroativo nunca reescreve a ficha com número velho. Excluir um abastecimento **não** devolve o odômetro: o carro rodou de verdade. Ao criar um quarto caminho, siga a mesma política e invalide `['veiculos']`/`['manutencoes']` do lado do front.
- **Dado que vem do cliente e não dá para recalcular pede trava de plausibilidade.** `Rota.DistanciaMetros`/`DuracaoEstimadaSegundos` chegam do front de propósito — recalcular no salvamento seria uma segunda chamada cobrada à Routes API. `RegrasDeTrajeto` recusa o par cuja velocidade média implícita fique fora de 5–180 km/h. Ao aceitar outro número que o servidor não possa conferir, siga o mesmo caminho: uma faixa larga que pegue o absurdo, mensagem sem jargão, e `OverridePropertyName` (não `WithName`) para o nome do campo não vazar na tela.
- **Valor derivado não entra no request.** `Abastecimento.Valor` é `Math.Round(litros × valorLitro, 2, MidpointRounding.AwayFromZero)`, calculado por `AbastecimentoMappings.CalcularValor` e recalculado em **toda** escrita — o `CreateAbastecimentoRequest` nem carrega o campo. É o que impede o resumo de custos de divergir do apontamento por payload manipulado ou bug de front.
...
- **Operação de conjunto no EF depois de projetar não traduz.** `Concat`/`Union` sobre um `Select` que produz constantes (`OrigemCusto.Abastecimento`, `"Combustível"`) falha com *"Unable to translate set operation after client projection"*, e ordenar por essas constantes falha também. Foi o que derrubou a primeira versão de `CustoRepository`: a união das origens é feita **em memória**, sobre páginas limitadas a `pagina × tamanhoPagina` de cada lado. `TraducaoDeConsultaTests` cobre os dois casos.
- **Listagem que cresce sem teto pagina; as demais, não.** Seis endpoints paginam (`/auditoria`, `/custo`, `/abastecimento`, `/despesa`, `/manutencao`, `/rota`); veículo, motorista, usuário, convite e catálogos continuam vindo inteiros de propósito — são limitados pela frota e alimentam os seletores do front. Ao paginar uma lista nova: request implementa `IRequestPaginado`, validator chama `this.AplicarRegrasDePaginacao()` (teto 100 num lugar só), filtro vira record em `Domain/Common` **sem `EmpresaId`**, e o repositório devolve `(Itens, Total)`. ⚠️ **A ordenação tem de ser total** — desempate por `Id` no fim do `OrderBy`, senão a página 2 repete linha da 1. ⚠️ **Recorte de segurança entra no filtro**, nunca depois da consulta: aplicado depois, o `COUNT` vaza o volume da empresa para quem só pode ver o próprio.
- **Repositório de read model é exceção, não regra.** `ICustoRepository` atravessa `Abastecimento`, `Manutencao`, `Despesa` e `Rota` porque serve uma consulta, não um agregado — e não escreve nada. Fora esse caso, mantenha um repositório por agregado.
- **Catálogo semeado tem duas fontes que precisam andar juntas.** `TiposManutencaoPadrao`/`TiposDespesaPadrao`/`TiposCombustivelPadrao` cobrem empresa nova via `BackofficeService`; as empresas já existentes foram semeadas por `INSERT` no `Up` da migration correspondente. `Posto` é a exceção: rede credenciada não tem padrão e **não** é semeada. A lista fica duplicada de propósito — migration é histórica e não acompanha a constante. Ao criar um catálogo novo, lembre dos **dois** caminhos: sem o da migration, todo cliente atual abre a tela com o seletor vazio.
- **Mapeamento é manual** via `ToResponse()`. AutoMapper está no csproj mas não é usado em lugar nenhum — não introduza `IMapper`.
- **Controllers**: `[Authorize]` na classe, `[ApiVersion("1.0")]`, rota `api/v{version:apiVersion}/[controller]`, `[Authorize(Roles = $"{Roles.Admin},{Roles.Supervisor}")]` por ação — **inclusive nos `DELETE`**: exclusão acompanha edição, quem cadastra também apaga. `Roles.Admin` sozinho sobrou só onde a tela inteira é dele (usuários, convites, auditoria); Operador cria/edita/encerra rota. Cada ação declara `[ProducesResponseType<ApiResponse<T>>(...)]` por status. Use `Roles.Gestao` (`Admin,Supervisor,Operador`) para barrar a role `Motorista` — atributos de classe e de ação são combinados por **E**, então não coloque `Roles.Gestao` na classe de um controller que também tenha ação aberta ao motorista.
