...
| Paginação | `ResultadoPaginado<T>` (`src/Domain/Common`) dentro de `ApiResponse<T>.Dados`; `GET /auditoria` e `GET /custo` paginam | `ResultadoPaginado<T>` em `types.ts` + o componente `Paginacao` de `components/Table.tsx` |
...
| Custos | **não há tabela de custos** — `ICustoRepository` é read model e une `Abastecimento.Valor` + `Manutencao.Custo` (só `Realizada` com custo) na leitura. `OrigemCusto` no Domain é o discriminador | a união `OrigemCusto` em `types.ts` espelha o enum — **mexeu numa, mexa na outra**. `/custos` é só gestão; `['custos']` é invalidada por abastecimento, manutenção **e encerramento de rota** (o km é o denominador do R$/km) |
